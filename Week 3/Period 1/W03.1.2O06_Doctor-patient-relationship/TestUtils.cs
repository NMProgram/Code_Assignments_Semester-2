...
static class TestUtils
{
    ...
    public static void PrintIsFieldListOfGenericType(Type clsType, string fieldName, Type expectedGenericArgument)
    {
        var field = clsType.GetField(fieldName,
            BindingFlags.Public | BindingFlags.NonPublic |
            BindingFlags.Instance | BindingFlags.Static);
        ...
        {
            Console.WriteLine($"(Class {clsType.Name}) field {fieldName} is not generic");
            return;
        }
        ...
        bool isList = genericTypeDefinition == typeof(List<>);  // Ensure it's a List<T>
        bool correctGenericArgument = genericArguments.Length > 0 && genericArguments[0] == expectedGenericArgument;

        Console.WriteLine($"(Class {clsType.Name}) field {fieldName} is a List<{expectedGenericArgument.Name}>: {isList && correctGenericArgument}");
    }

    public static void PrintIsFieldTypeArray(Type clsType, string fieldName)
    {
        var field = clsType.GetField(fieldName,
            BindingFlags.Public | BindingFlags.NonPublic |
            BindingFlags.Instance | BindingFlags.Static);
        ...
        if (fieldType.IsArray)
        {
            ...
            {
                Console.WriteLine($"{fieldName} is a jagged array.");
            }
            else if (fieldType.GetArrayRank() > 1)
            {
                Console.WriteLine($"{fieldName} is a multidimensional array.");
            }
            else
            {
                Console.WriteLine($"{fieldName} is a one-dimensional array.");
            }
        }
        else
        {
            Console.WriteLine($"{fieldName} is not an array.");
        }
    }
    ...
    {
        MethodBody body = method.GetMethodBody() ?? throw new ArgumentException("Unexpected args value: {methodName}", method.Name);
