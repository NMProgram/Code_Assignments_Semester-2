# Overview
This exercise requires the programmer to make the classes `Season`, `Driver`, `Team` and `Race` 
to randomly generate a Racing Season with points being awarded depending on the position of the Driver instance.

Only the classes mentioned were coded by me, the rest was provided as a template by CodeGrade.

This took me a while to do, mainly because CodeGrade gave me barely anything to work with and explained the details of each class in an illogical order.
But, having done it now, it wasn't so bad. 
One thing I am confused about though is that I was forced to sort a list in `C#` when that hasn't even been formerly introduced yet.
But other than that, it was fine.

# Code
Other files can be found in this directory (including mentioned classes).
```cs
